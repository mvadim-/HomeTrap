...
- `README.md` — описано разові знакові коригування, авто-витрату, її read-only
  життєвий цикл і симетричне включення до P&L після виставлення рахунка.
- `CLAUDE.md` — зафіксовано інваріанти adjustment-ліній, зв'язку
  `Expense.invoice_line_id`, CASCADE та draft-фільтра P&L.
- `docs/improvements-backlog.md` — завершене покращення додано до реалізованих
  можливостей і до підсумку білінгу/P&L; Task 10 у плані закрито, а terminal move
  плану відкладено оркестратору до завершення review-фаз.
- Зміна призначена для production, але автоматичний деплой не виконувався. Вона
  містить міграцію `20260722_09`: перед розгортанням потрібен ручний DR-архів
  усього `data/`, потім rebuild/restart за `docs/deploy.md`; міграція
  застосовується автоматично під час старту, сервіс лишається з одним worker.
