...
- [x] парсер XLSX (openpyxl): вкладка «Загальна інформація» → послуги + тарифи з датами колонок; вкладки «<Місяць> <Рік>» → рахунки (показники, фіксовані суми, курс, оренда) зі статусом `paid`
- [x] `POST /api/apartments/{id}/import` (upload файлу, python-multipart) + `dry_run=true` для попереднього перегляду
- [x] звіт імпорту: створено N рахунків, пропущено/попередження (нечислові клітинки, розриви місяців)
- [x] ідемпотентність: повторний імпорт не дублює наявні періоди (unique apartment+period)
- [x] фікстура з артефактами експорту (об'єднані клітинки, «7.95689грн.», прочерки «-»); ⚠️ реальний XLSX відсутній у репозиторії, тому використано анонімізовану синтетичну структуру сумісності
- [x] тести на фікстурі: повний імпорт, dry run, повторний імпорт, биті клітинки, об'єднані клітинки
- [x] прогнати тести — мають пройти перед таском 10
...
- [x] відправники: Telegram Bot API (token + chat_id) і SMTP; кожен канал вмикається окремо
- [x] правила (щоденна задача, `Europe/Kyiv`): у день X місяця — «зняти показники» по активних квартирах; issued-рахунок не оплачено ≥ N днів — нагадування з повтором кожні M днів
- [x] `GET/PUT /api/settings` (збереження в `Setting`), `POST /api/settings/test-notification`
- [x] тести з mock-відправниками: спрацювання правил, вимкнені канали, повтор без дублю в той самий день
- [x] прогнати тести — мають пройти перед таском 11
...
- [x] Дашборд: плитки (нараховано, оплачено, заборгованість, курс), список квартир зі статусами, блок «потребує уваги» (з `/api/stats/dashboard`)
- [x] Квартири: список; картка квартири: реквізити, оренда, послуги з тарифами й «діє з», форми додавання/редагування послуг і тарифів
- [x] кнопка-заглушка «Посилання орендаря» (майбутня фіча, disabled із тултипом)
- [x] тести: рендер дашборда з mock-даними, картка квартири з таблицею послуг
- [x] прогнати тести — мають пройти перед таском 13
...
- [x] список рахунків: фільтри по квартирі/статусу, статусні плашки (чернетка/виставлений/оплачений/прострочений)
- [x] створення/редагування чернетки: попередні показники read-only, введення поточних, живий перерахунок сум і підсумків, редагований курс, показ попереджень валідації
- [x] дії: виставити, повернути в чернетку, позначити оплаченим (з датою)
- [x] тести: InvoiceCalculator — перерахунок при зміні показника й курсу, показ попередження при показнику меншому за попередній
- [x] прогнати тести — мають пройти перед таском 14
...
- [x] графіки споживання: окремі міні-графіки газ/світло/вода (SVG, кольори й підписи з макета), тултіпи
- [x] графік доходу: стек оренда+комуналка помісячно; перемикач квартира/портфель
- [x] тести: рендер графіків з mock-даними, порожня історія без падіння
- [x] прогнати тести — мають пройти перед таском 15
...
- [x] Налаштування: Telegram/SMTP/дні нагадувань, кнопка тест-повідомлення
- [x] секція імпорту XLSX: upload, dry-run прев'ю, звіт імпорту з попередженнями
- [x] тести: рендер форми налаштувань, показ звіту dry-run з mock-відповіді
- [x] прогнати тести — мають пройти перед таском 16
