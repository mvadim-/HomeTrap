...
- Порядок списку: бекенд `list_tenants` вже повертає активного першим, далі за
  ...
  прихована. Помилка `getTenants` → випадайка прихована, без error-банера
  (функція опційна), а значення простою позначене недоступним. Під час
  завантаження числовий простій також не показується.
...
- [x] обробник вибору: `periodMode="custom"`, `dateFrom` = місяць
  `contract_start`, `dateTo` = місяць `contract_end` або поточний місяць у
  `Europe/Kyiv`; для майбутнього активного договору `dateTo` = місяць початку
