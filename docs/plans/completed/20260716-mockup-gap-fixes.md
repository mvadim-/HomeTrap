...
- [x] API-клієнт: типи й методи tenants/attachments (multipart)
- [x] картка активного орендаря: ПІБ, телефон, email, контракт з дати; редагування; «Завершити контракт» (дата, confirm)
- [x] «Новий орендар»: форма доступна лише коли активного немає (після завершення контракту); 409 з бекенда показується зрозуміло
- [x] файли контракту: завантаження кількох, список з original_name, відкриття в новій вкладці, видалення з confirm
- [x] історія колишніх орендарів (згорнутий блок: ПІБ, період контракту)
- [x] тести: рендер активного орендаря, форма нового після завершення, список файлів, помилка 409
...
- [x] перемикач періоду: чіпи «6 міс / 12 міс / 24 міс» (months) + «Весь час» (`all_time=true`) + режим «Довільний період» (два input type="month" → date_from/date_to); спільний для споживання й доходу
- [x] три саммарі-тайли: «Оренда за період» і «Комунальні за період» з наявного income.totals, «Найбільша стаття» з income.top_service (назва · частка % · «пік — <місяць>»); стан без даних
- [x] підписи сум над стовпчиками доходу (як у макеті; компактний формат «16,7»)
- [x] підзаголовок сторінки відображає обраний період замість жорстких «12 місяців»
- [x] тести: перемикання пресета змінює запит, довільний період, рендер тайлів, підписи над стовпчиками
- [x] прогнати frontend-тести — мають пройти перед таском 8

### Task 8: Verify acceptance criteria

- [x] повний сценарій у Docker: створити орендаря → прикріпити 2 файли → переглянути → завершити контракт → додати нового → історія обох; статистика: пресети, довільний період, тайли (інтегрований API acceptance-тест + UI Vitest)
- [x] крайні випадки: квартира без орендаря, орендар без файлів, статистика без даних у діапазоні (backend acceptance + явні UI empty-state перевірки)
- [x] повний тест-сьют: backend pytest + Ruff + frontend Vitest + build — усе зелене (78 pytest, Ruff, 48 Vitest, Vite production build у Docker)
- [x] звірити з макетом: блок орендаря, тайли статистики, підписи стовпчиків (живий Docker UI через Playwright + DOM-перевірки непорожніх графіків)
