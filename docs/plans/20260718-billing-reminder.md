...
- [x] таблиця: квартира, орендар, дата, статус рахунка; сортування за датою, горизонт 30 днів
- [x] підсвітка рядків без рахунка після дати виставлення (warning-токен `theme.css`); клік → сторінка квартири/рахунка
- [x] write tests: рендер, сортування, підсвітка проблемних, порожній стан, помилка API
- [x] run tests - must pass before next task
