...
- [ ] завантажувати орендарів вибраної квартири через `getTenants` (ефект на
  зміну `apartmentId`, з cleanup за зразком наявних ефектів); порядок
  використовуємо як є з API (без клієнтського сортування)
- [ ] випадайка з `aria-label="Орендар для статистики"`: суфікс «(поточний)»
...
- [ ] обробник вибору: `periodMode="custom"`, `dateFrom` = місяць
  `contract_start`, `dateTo` = місяць `contract_end` або поточний місяць
- [ ] додати дефолтний мок `getTenants` (порожній список) до НАЯВНИХ тестів
  `Stats.test.tsx`, щоб новий ефект не робив реальних fetch-викликів і не
  давав act()-попереджень
- [ ] write tests: вибір орендаря → custom-режим, поля заповнені, API отримує
  `date_from`/`date_to` договору; активний орендар → `dateTo` = поточний
  місяць і позначка «(поточний)»
- [ ] write tests: зміна квартири → нові орендарі; квартира без орендарів →
...
- [ ] обчислювати значення випадайки зіставленням поточного custom-діапазону з
  місяцями договорів (перший збіг); без збігу — плейсхолдер «—»
- [ ] інфо-рядок під панеллю періоду при активному збігу: «Договір:
  {dd.mm.yyyy} — {dd.mm.yyyy|досі} · {активний|завершений}»
- [ ] write tests: ручна зміна дати → випадайка «—», інфо-рядок зникає;
