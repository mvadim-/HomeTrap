...
- [x] сторінка `Expenses.tsx`: форма (квартира або «загальна», дата, категорія,
...
- [x] маршрут у `App.tsx` та пункт навігації в `Layout.tsx`
- [x] write tests: створення/редагування/видалення; загальна витрата;
...
- [x] YoY-накладення на `MiniLineChart` (пунктир «той самий місяць торік»)
...
- [x] дельта vs попередній місяць і vs той самий місяць торік (стрілка/%,
  доступність через `aria-label`); плитки/підпис `avg/min/max` по послузі
- [x] перемикач одиниць ↔ ₴ (вартість спожитого з `cost`)
- [x] нові кольори/стани — токени в усі три блоки `theme.css`
- [x] write tests: YoY-лінія присутня/відсутня залежно від діапазону; дельта
